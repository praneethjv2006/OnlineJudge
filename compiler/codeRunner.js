...
const runCodeAgainstTestCases = async ({ code, language, testCases, timeLimitMs = 2000 }) => {
  ...
  try {
    ...
    results.forEach((res, index) => {
      terminalLines.push(`=== Case ${res.id} ===`);
      if (res.input) terminalLines.push(`Input:\n${res.input}`);
      if (res.stdout) terminalLines.push(`Output:\n${res.actualOutput}`);
      else terminalLines.push("Output:\n(no stdout)");
      if (res.stderr) terminalLines.push(`Stderr:\n${res.stderr.trimEnd()}`);
      terminalLines.push(`Verdict: ${res.verdict}\n`);
    });
