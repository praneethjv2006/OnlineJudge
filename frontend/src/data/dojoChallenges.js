...
export const CATEGORY_INFO = {
  ...
  "Fix the Performance": {
    title: "Fix the Performance (TLE Fix)",
...
export const CHALLENGES = [
  ...
  // 3. PRECISION TRAP
  {
    ...
    starterCode: {
      ...
      c: `#include <stdio.h>\n#include <math.h>\n\nint main() {\n    long long r;\n    scanf("%lld", &r);\n    double side = r * sqrt(2.0);\n    double area = side * side;\n    printf("%.6f\\n", area);\n    return 0;\n}`,
      python: `import math\nr = int(input())\nside = r * math.sqrt(2)\narea = side * side\nprint(f"{area:.6f}")`,
      javascript: `const r = Number(require("fs").readFileSync(0,"utf8").trim());\nconst side = r * Math.sqrt(2);\nconst area = side * side;\nconsole.log(area.toFixed(6));`,
    },
    uiType: "blind",
...
export function pickChallenge(excludeId = null) {
  ...
  const weights = CHALLENGES.map((c, i) => {
    if (excludeId && c.id === excludeId) return 0;
  ...
  if (total <= 0) {
    ...
    const fallbackIdx = CHALLENGES.findIndex((c) => c.id !== excludeId);
    return CHALLENGES[fallbackIdx >= 0 ? fallbackIdx : 0];
  }
